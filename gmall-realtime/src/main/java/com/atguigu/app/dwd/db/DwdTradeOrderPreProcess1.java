...
public class DwdTradeOrderPreProcess1 {
    public static void main(String[] args) throws Exception {
        ...
        //todo 2,从KAFKA  topic_db中读取数据并创建原始表
        tableEnv.executeSql(MyKafkaUtil.getTopicDB("order_pre_process_220212"));

        //todo  3,过滤出订单明细表(不熟悉业务的情况下,全取没坏处)   注意自取的列名与后面即将调用的保持一致
        ...
        tableEnv.createTemporaryView("oeder_info", orderInfo);
        //todo  5,过滤出订单明细活动表
        ...
        //todo  7,构建MYsql的base_dic  LookUp表
        tableEnv.executeSql(MySqlUtil.getBaseDic());
        //todo  8,将五张表关联成为预处理表   为接下来的流量域的其他表方便调用
        ...
        tableEnv.executeSql("" +
                ...
                "    `old` MAP<STRING, STRING>, " +       //注意这里的MAP类型
                "    `activity_id` STRING, " +
