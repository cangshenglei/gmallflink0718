...
//        tableEnv.getConfig().setLocalTimeZone(ZoneId.of("GMT+8"));
        //TODO 2,读取KAFAK ODS层 topic_db 业务数据
        String topic = "topic_db";
        String groupId = "province_order_220212";
        DataStreamSource<String> kafkaDS = env.addSource(MyKafkaUtil.getFlinkKafkaConsumer(topic, groupId));

        //TODO 3,过滤数据 去下单shuju
        SingleOutputStreamOperator<String> filterDS = kafkaDS.filter(new FilterFunction<String>() {
            @Override
            public boolean filter(String s) throws Exception {
                JSONObject jsonObject = JSON.parseObject(s);
                return "oeder_info".equals(jsonObject.getString("table")) && "insert".equals(jsonObject.getString("type"));
            }
        });

        //TODO 4,将数据转为JAVABean对象
        ...
        })).keyBy(TradeProvinceOrderWindow::getProvinceId).window(TumblingEventTimeWindows.of(Time.seconds(10)))
                .reduce(new ReduceFunction<TradeProvinceOrderWindow>() {
                    @Override
                    public TradeProvinceOrderWindow reduce(TradeProvinceOrderWindow tradeProvinceOrderWindow, TradeProvinceOrderWindow t1) throws Exception {
                        tradeProvinceOrderWindow.setOrderCount(tradeProvinceOrderWindow.getOrderCount() + t1.getOrderCount());
                        tradeProvinceOrderWindow.setOrderAmount(tradeProvinceOrderWindow.getOrderAmount() + t1.getOrderAmount());
        ...
        SingleOutputStreamOperator<TradeProvinceOrderWindow> tradeProvinceOrderWithProvinceDS = AsyncDataStream.unorderedWait(
                reduceDS, new DimAsyncFunction<TradeProvinceOrderWindow>("DIM_BASE_PROVINCE") {
                    @Override
