...
public class Flink02_SQLJoin {
    public static void main(String[] args) {
        ...
        SingleOutputStreamOperator<Bean1> bean1DS = env.socketTextStream("hadoop102", 8888).map(line -> {
            String[] split = line.split(",");
            return new Bean1(split[0], split[1], Long.parseLong(split[2]));
        });

        SingleOutputStreamOperator<Bean2> bean2DS = env.socketTextStream("hadoop102", 9999).map(line -> {
            String[] split = line.split(",");
        ...
        //TODO 内连接 左表为OnCreateAndWrite           右表:OnCreateAndWrite
        //      tableEnv.sqlQuery("select t1.id,t1.name,t2.sex from t1 join t2 on t1.id=t2.id")
        //            .execute().print();

        //todo 左外连接    左表为:OnReadAndWrite      右表:OnCreateAndWrite
        Table resultTable = tableEnv.sqlQuery("select t1.id,t1.name,t2.sex from t1 left  join t2 on t1.id=t2.id");
        tableEnv.createTemporaryView("result_table", resultTable);

        //TODO 右外连接       左表为:OnCreateAndWrite      右表:OnReadAndWrite
        //    tableEnv.sqlQuery("select t2.id,t2.sex,t1.name from t1 right join t2 on t1.id=t2.id ").execute().print();


        //TODO 全外连接        左表为:OnReadAndWrite      右表:OnReadAndWrite
        //     tableEnv.sqlQuery("select t1.id id_1,t2.id id_2 from t1 full join t2 on t1.id = t2.id")
//                .execute()
//                .print();

        //flinkSQL从mysql中直接读取 ,并关联
        tableEnv.executeSql("" +
                "CREATE TABLE test ( " +
                "  `id` STRING, " +
                "  `name` STRING, " +
